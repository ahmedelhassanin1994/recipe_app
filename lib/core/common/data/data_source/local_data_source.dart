...
abstract class LocalDataSource {
  Future<Box> Openbox();
  Future<Model_Cache> getCache(String key);
...
class LocalDataSourceImplementer implements LocalDataSource {
  ...
  @override
  Future<Model_Cache> getCache(String key) async {
    // TODO: implement getCache
    final box = await Openbox();
    List<Model_Cache> model = box.values.cast<Model_Cache>().toList();
    Model_Cache model_cache=  model.where((element) => element.key.toString().contains("$key")).first;
    return model_cache;
  }
}
